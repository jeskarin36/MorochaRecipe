...
const MyRecipes = () => {
    ...
    const cuisines = ['Todas', 'Italiana', 'Mexicana', 'India', 'China', 'Japonesa', 'Tailandesa', 'Francesa', 'Mediterránea', 'Americana'];
    const difficulties = ['Todas', 'easy', 'medium', 'hard']; // Se mantienen los valores originales para la API, la traducción se hace en el renderizado
    ...
    const filterRecipes = () => {
        ...
        if (selectedCuisine !== 'Todas') {
            // Mapeo inverso simple para asegurar compatibilidad si la API devuelve el tipo en inglés
            const cuisineMap = {
                'Italiana': 'Italian', 'Mexicana': 'Mexican', 'India': 'Indian', 
                'China': 'Chinese', 'Japonesa': 'Japanese', 'Tailandesa': 'Thai', 
                'Francesa': 'French', 'Mediterránea': 'Mediterranean', 'Americana': 'American'
            };
            const targetCuisine = cuisineMap[selectedCuisine] || selectedCuisine;
            filtered = filtered.filter(recipe => recipe.cuisine_type === targetCuisine || recipe.cuisine_type === selectedCuisine);
        }
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="mb-6">
                    ...
                    <p className="text-gray-600 mt-1">Tu colección de recetas guardadas</p>
                </div>
                    ...
                    <div className="flex flex-col lg:flex-row gap-4">
                        ...
                        >
                            {difficulties.map(diff => {
                                let diffLabel = 'Todas las dificultades';
                                if (diff === 'easy') diffLabel = 'Fácil';
                                if (diff === 'medium') diffLabel = 'Media';
                                if (diff === 'hard') diffLabel = 'Difícil';
                ...
                {filteredRecipes.length > 0 ? (
                    ...
                    <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
                        ...
                        <p className="text-gray-500 mb-4">
                            {recipes.length === 0 ? 'Aún no tienes recetas' : 'Ninguna receta coincide con tus filtros'}
                        </p>
                        {recipes.length === 0 && (
                            <Link
                                to="/generate"
...
const RecipeCard = ({ recipe, onDelete }) => {
    ...
    // Mapeo visual para las etiquetas de dificultad de las tarjetas
    ...
    // Mapeo visual para los tipos de cocina si vienen en inglés desde la API
    const cuisineLabels = {
        'Italian': 'Italiana', 'Mexican': 'Mexicana', 'Indian': 'India', 
        'Chinese': 'China', 'Japanese': 'Japonesa', 'Thai': 'Tailandesa', 
        'French': 'Francesa', 'Mediterranean': 'Mediterránea', 'American': 'Americana'
    };
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                    {recipe.cuisine_type && (
                        <span className="px-2 py-1 bg-emerald-100 text-emerald-700 rounded text-xs font-medium">
                            {cuisineLabels[recipe.cuisine_type] || recipe.cuisine_type}
                        </span>
                    )}
                    {recipe.difficulty && (
