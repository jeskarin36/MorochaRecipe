...
export const generateRecipe = async (req, res, next) => {
    try {
        const { ingredients = [], usePantryIngredients = false, dietaryRestrictions = [], cuisineType = "any", serving = 4, cookingTime = "medium" } = req.body;
...
export const getPantrySuggestions = async (req, res, next) => {
    try {
        ...
        const suggestions = await generatePantrySuggestionsAI(expiringNames);
...
export const getRecipes = async (req, res, next) => {
    try {
        ...
        const recipes = await Recipe.findUserId(req.user.id, {
            search, cuisine_type, difficulty, dietary_tag, max_cook_time: max_cook_time ? parseInt(max_cook_time) : undefined, sort_by, sort_order, limit: limit ? parseInt(limit) : undefined, offset: offset ? parseInt(offset) : undefined
        });
...
export const getRecipeStats = async (req, res, next) => {
    try {
        const stats = await Recipe.getStatus(req.user.id);
