...
class MealPlan {
    static async create(userId, mealData) {
        // Asegurado que use 'meal_plans' en plural como el resto del archivo
        const { recipe_id, planned_date, meal_date, meal_type } = mealData;
        const date = planned_date || null;
    ...
    static async findByDateRange(userId, startDate, endDate) {
        // Corregido: ORDER_BY -> ORDER BY, cambiado el segundo >= por <=, y corregido 'breakfast'
        const result = await db.query(
            `SELECT mp.id, mp.user_id, mp.recipe_id, mp.meal_date::text as meal_date, 
                    mp.meal_type, mp.created_at, mp.updated_at, r.name as recipe_name, 
