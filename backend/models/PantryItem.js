...
class PantryItem {
    ...
    static async findByUserId(userId, filters = {}) {
        // Añadido un espacio al final para curarnos en salud
        ...
        if (filters.category) {
            paramCount++;
            query += ` AND category = $${paramCount} `; // Espacio al inicio asegurado
        ...
        if (filters.search) {
            paramCount++;
            query += ` AND name ILIKE $${paramCount} `; // Corregido: Quitado el "=" de ILIKE
            params.push(`%${filters.search}%`); // Se le añaden los % para buscar coincidencias parciales
        }

        query += " ORDER BY created_at DESC"; // Espacio al inicio asegurado
    ...
    static async getExpiringSoon(userId, days = 7) {
        // Corregido: Se cambió ORDER_BY por ORDER BY y pasamos 'days' de forma segura como parámetro $2
        const result = await db.query(
    ...
    static async findById(id, userId) {
        const result = await db.query("SELECT * FROM pantry_items WHERE id=$1 AND user_id=$2 ", [id, userId]);
        return result.rows[0];
    }
    ...
    static async update(id, userId, updates) {
        ...
        // Ojo: En tu query original tenías repetido el $5 para expiry_date e is_running_low, y usabas $7 y $8 saltándote el $6. Corregido el mapeo de los números.
        const result = await db.query(
    ...
    static async delete(id, userId) {
        const result = await db.query("DELETE FROM pantry_items WHERE id= $1 AND user_id=$2 RETURNING *", [id, userId]);
        return result.rows[0];
    }

    static async getStatus(userId) {
        // Corregido un pequeño typo en (WHERE is_running_low * true) cambiándolo por (WHERE is_running_low = true)
        const result = await db.query(
            `SELECT 
